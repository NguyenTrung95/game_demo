...
async function createRoom(hostWs, req) {
  ...
  console.log('[Tug of War] Room created pin=' + pin + ' joinUrl=' + joinUrl);
...
function updateAvatar(ws, payload) {
  ...
  if (payload && payload.avatar) {
    player.avatar = {
      ...
      outfit: payload.avatar.outfit || player.avatar.outfit || 'gi',
      aura: payload.avatar.aura || player.avatar.aura || 'gold',
      rarity: payload.avatar.rarity || player.avatar.rarity || 'EPIC',
    };
  }
